...
func NewConsumer(amqpURI, exchange, exchangeType, queueName, key, ctag string) (*Consumer, error) {
	...
	queue, err := c.channel.QueueDeclare(
		...
		true,      // durable
		false,     // delete when unused // 当所有消费客户端连接断开后，是否自动删除队列
		false,     // exclusive //仅创建者可以使用的私有队列，断开后自动删除
		false,     // noWait
		nil,       // arguments
	)
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("Qos err: %s", err)
	}
	...
	for d := range deliveries {
		...
		go func() {
			defer func() {
				err := recover()
				if err != nil {
					log.Printf("捕获异常: %s, tag: %d", err, d.DeliveryTag)
				}
				d.Nack(false, true)
			}()
			time.Sleep(1 * time.Second)
			panic(errors.New("手动异常"))
			log.Printf("处理业务结束")
			//d.Nack(true, true)
		}()
	}
